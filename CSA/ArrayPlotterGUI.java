    ...
    implements GridChangeListener
{
    ...
    private boolean filledArrayCreated;
    ...
    public void reactToNewGrid(Grid<GridObject> grid)
    {
        ...
        display.plotter.initializeFilledArray(
            grid.getNumRows(), grid.getNumCols());
        filledArrayCreated = true;
    ...
    public void update(boolean[][] ca)
    {
        ...
        // THE ArrayPlotter filledArray instance variable
        boolean[][] filledArray = getFilledArray();
        if (ca != filledArray)
        throw new IllegalStateException("Array passed to update " +
                "is not the ArrayPlotter filledArray instance variable.");
        ...
        if (Arrays.deepEquals(ca, previousCA))
        {
            ...
            //              null,
            //              "No filledArray elements have changed value since last update." +
    ...
    {
        // Retrieve reference to array plotter's filledArray
        boolean[][] filledArray = getFilledArray();

        Grid<GridObject> g = getGrid();

        if (filledArrayCreated)
        {
            if (filledArray == null)
                throw new IllegalStateException("filledArray is null!");
            if (g.getNumRows() != filledArray.length)
                throw new IllegalStateException(
                    "filledArray row count is incorrect!");
            if (g.getNumCols() != filledArray[0].length)
                throw new IllegalStateException(
                    "filledArray col count is incorrect!");
        }
        else
            return;
    ...
    /** Gets the Array Plotter's filledArray instance variable. */
    private boolean[][] getFilledArray()
    {
        Field filledArrayField = null;
        try
        {
            filledArrayField = ArrayPlotter.class.getDeclaredField("filledArray");
        }
        catch (NoSuchFieldException e)
        {
            System.err.println("ArrayPlotter filledArray - NoSuchFieldException");
        }

        filledArrayField.setAccessible(true);

        boolean[][] filledArray = null;
        try
        {
            filledArray = (boolean[][])filledArrayField.get(display.plotter);
        }
        catch (IllegalAccessException e)
        {
            System.err.println("ArrayPlotter filledArray - IllegalAccessException");
        }
